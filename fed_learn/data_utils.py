...
class DataHandler:
    ...
    def assign_data_to_clients_plus(self, clients: List[Client], sampling_technique: str, proportion = 0.01):
        sampled_data_indices = self._sample(sampling_technique, len(clients))
        #all device receive data
        for client, data_indices in zip(clients[:-1], sampled_data_indices):
            x = self.x_train[data_indices]
            y = self.y_train[data_indices]
            x, y = self.preprocess(x, y)
            client.receive_data(x, y)
        ...
        for digit in range(10):
            all_samples = [i for i, d in enumerate(self.y_train) if d == digit]
        ...
        #the last device receive shared data
        x = self.x_train[z]
        y = self.y_train[z]
        x, y = self.preprocess(x, y)
        clients[-1].receive_data(x, y)
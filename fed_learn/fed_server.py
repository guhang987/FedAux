...
class Server:
    ...
    def create_clients_plus(self):
        # Create all the clients
        for i in range(self.nb_clients + 1):
            client = fed_learn.Client(i)
            self.clients.append(client)

    def summarize_weights(self,epoch,test_acc):
        new_weights = self.weight_summarizer.process(self.client_model_weights,epoch,test_acc)

...
x_share,y_share = fed_learn.get_shared_data(x_train,y_train,0.001)
...
#split train set and test set from shared data
index1 = []
...
model_weights = "./model/aux_model_fmnist"

checkpoint = ModelCheckpoint(model_weights, monitor='val_loss', verbose=0, save_best_only=True, mode='min',
                             save_weights_only=True)
a_model = fed_learn.create_model_cnn((28, 28, 1), NUMBER_OF_CLASS, init_with_imagenet=False,learning_rate=0.001)
...
a_model.save_weights(model_weights, overwrite=True)

results = a_model.evaluate(x_test, y_test, batch_size=50, verbose=1)
